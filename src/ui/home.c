...
lv_obj_t * create_app_list_tile(lv_obj_t * parent, u_int8_t col, u_int8_t row, lv_dir_t direction) {
  ...
  ui_app_list_tile = lv_tileview_add_tile(parent, col, row, direction);

  ui_col_container = new_ui_flex_container(ui_app_list_tile, LV_ALIGN_CENTER, LV_FLEX_FLOW_COLUMN, LV_FLEX_ALIGN_START,
  ...
  lv_obj_add_event_cb(ui_gallery_btn, open_gallery_event_cb, LV_EVENT_CLICKED, &gallery_init);
...
lv_obj_t * create_app_tile(lv_obj_t * parent, u_int8_t col, u_int8_t row, lv_dir_t direction) {
  ...
  lv_obj_clear_flag(ui_app_tile, LV_OBJ_FLAG_SCROLLABLE);
...
lv_obj_t * home_screen(void) {
  ...
  lv_obj_set_scrollbar_mode(System.main_tile_view, LV_SCROLLBAR_MODE_OFF);
